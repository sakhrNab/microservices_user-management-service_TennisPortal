...
from apps.manage_user.controller.serializers\
    .profile_management.serializers\
    import UserSerializer, WishListSerializer
...
class GetSingleUserTest(TestCase):
    ...
    #     profile = Profile.objects.get(pk=9)
    #     # profile2 = Profile.objects.get(username="useranme1")
    #     serializer = UserSerializer(user)
    #     serializer2 = ProfileSerializer(profile)
    #     print("response data: ", response.data) # profile
    #     print("serializer data: ", serializer.data)
    #     print ("profile data: ", profile)
    #     print("serializer 2: ", serializer2)
    #     # print ("profile data 2: ", profile2)
...
class PasswordResetTest(TestCase):
    ...
    def test_create_valid_password_reset(self):
        response = client.post(
            reverse('apps.manage_user:reset_password'),
...
#         response = c.post(
#             reverse('apps.manage_user:changepassword', kwargs={'pk': int(self.user1.pk)}),
#             data=json.dumps(self.valid_payload),
#             content_type='application/json'
#         )
#         self.assertEqual(response.status_code, status.HTTP_200_OK)
...
#         c.force_login(self.user1)
#         response = c.post(
#             reverse('apps.manage_user:reset_password', kwargs={'pk': int(self.user1.pk)}),#
#             data=json.dumps(self.invalid_payload),
#             content_type='application/json'
#         )
...
#             reverse('apps.manage_user:delete_user', kwargs={'username': 'zackazico'}),
#             content_type='application/json',
#             follow=True
#         )
#         self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class LatestUsersTest(TestCase):
    ...
    def test_get_popular_users(self):
        ...
        self.assertEqual(sorted(response.data), sorted(new_serializer))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
#         c.force_login(self.user1)
#
#         response = c.get(reverse('apps.manage_user:recommended_players'))
#
#         recommended_users = Profile.objects.filter(Q(age=self.user1.profile.age)
#                                                    | Q(region=self.user1.profile.region)
#                                                    | Q(gender=self.user1.profile.gender)
#                                                    | Q(skill_level=self.user1.profile.skill_level)).exclude(Q(user__pk=self.user1.profile.pk) | Q(user__available=False))
