...
class UpdateAvailablityTest(TestCase):
    ...
    def test_available(self):
        ...
        url = reverse('apps.manage_user:update_profile', kwargs={'pk': int(self.user1.pk)})
        content_type='application/json'
...
class DestroyUserTest(TestCase):
    ...
    def test_destroy_user(self):
        c = Client()
        c.force_login(self.user1)
        url = reverse('apps.manage_user:delete_user', kwargs={'username': str(self.user1.username)})
        content_type='application/json'
        access = AccessToken.for_user(self.user1)
        bearer = {"HTTP_AUTHORIZATION": f'Bearer {access}'}

        response = c.delete(url, content_type, follow=True, **bearer)
...
class RecommendedUsersTest(TestCase):
    ...
    def test_get_recommended_users(self):
        ...
        url = reverse('apps.manage_user:recommended_players')
        access = AccessToken.for_user(self.user1)
        bearer = {"HTTP_AUTHORIZATION": f'Bearer {access}'}

        response = c.get(url, **bearer)
...
class WishListViewSetTest(TestCase):
    ...
    def test_all_wishlist_users(self):
        c = Client()
        c.force_login(self.user1)
        url = reverse('apps.manage_user:wishlist')
        access = AccessToken.for_user(self.user1)
        bearer = {"HTTP_AUTHORIZATION": f'Bearer {access}'}
