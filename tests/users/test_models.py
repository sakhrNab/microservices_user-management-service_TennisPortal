...
def test_super_user_is_not_superuser(user_factory):
    """Test that an error is raised when the admin user has is_super
    set to false"""
    with pytest.raises(ValueError) as err:
...
def test_create_user_with_no_email(user_factory):
    """Test that the creation of a new user without an email
    raises error"""
    with pytest.raises(ValueError) as err:
...
def test_create_user_with_no_username(user_factory):
    """Test tjat a creation of a new user without username
    raises an error"""
    with pytest.raises(ValueError) as err:
...
def test_create_user_with_no_firstname(user_factory):
    """Test tjat a creation of a new user without firstname
    raises an error"""
    with pytest.raises(ValueError) as err:
...
def test_create_user_with_no_lastname(user_factory):
    """Test tjat a creation of a new user without lastname
    raises an error"""
    with pytest.raises(ValueError) as err:
...
def test_create_superuser_with_no_password(user_factory):
    """Test tjat a creation of a new superuser without
    an password raises an error"""
    with pytest.raises(ValueError) as err:
        user_factory.create(password=None, is_superuser=True,
                            is_staff=True)
...
def test_user_email_incorrect(user_factory):
    ...
    assert str(err.value) == "You must provide a valid email addresss"